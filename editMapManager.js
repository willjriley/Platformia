...
export function loadMapData(map) {
    ...
    console.log("Entities:", entitiesCollection);
...
export function saveMap() {
    const map = {
        ...
        gradientMiddle,
        gradientBottom,
        music: currentMusic ? currentMusic.src : null,
        playerStartingPosition: {
            x: player.startX,
            y: player.startY
        },
        entities: entitiesCollection.map(entity => ({
            type: entity.constructor.name.toLowerCase(),
            x: entity.x,
            y: entity.y,
            // Add other properties as needed
        })),
        particles: particleEmitters.map(emitter => ({
            type: emitter.constructor.name.toLowerCase(),
            x: emitter.x,
            y: emitter.y,
            // Add other properties as needed
        })),
        backgroundImages: backgroundImages.map(bgImage => ({
            x: bgImage.x,
            y: bgImage.y,
            width: bgImage.width,
            height: bgImage.height,
            imageSrc: bgImage.imageSrc
        }))
    };
